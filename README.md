 ...
 * 중앙 집중식 캐시 시스템 (PlayerJsonManager):
   * 모든 플레이어 데이터는 중앙 집중식 메모리 캐시에 보관되며, 플레이어 접속 시 로드(loadPlayerJson)되고, 접속 종료 시 즉시 저장(savePlayerJson)되어 데이터 유실을 최소화합니다.
 * 커맨드-캐시 동기화 강제 (Critical Stability):
   * 관리자 명령어(예: /DRPG setLevel, /DRPG addExp) 사용 후 데이터 불일치(Desync)가 발생하지 않도록, 모든 데이터 수정 로직은 중앙 캐시를 통해서만 이루어지며, 수정 직후 디스크에 즉시 저장하는 로직을 강제하였습니다. 이는 서버 재부팅 시 발생할 수 있는 데이터 롤백 가능성을 완벽하게 차단합니다.
 * 비동기 I/O 및 성능 최적화:
