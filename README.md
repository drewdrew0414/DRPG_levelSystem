...
**이 플러그인은 Minecraft 서버에 최고 수준의 안정성, 확장성, 디버깅 편의성을 제공하는 RPG 스킬 레벨링 시스템입니다.**

* All core logic, including skill triggers, experience curves, and rewards, is fully customizable via `.json` files. Server operators can design deep, complex RPG content without writing a single line of Java code.
* 모든 핵심 로직(스킬 트리거, 경험치 곡선, 보상 포함)은 `.json` 파일로 완벽히 커스텀마이징 가능합니다. 이는 서버 운영의 복잡성을 획기적으로 낮춥니다.

---

## 1. Core Features / 핵심 기능

**This plugin fundamentally focuses on solving common problems faced by existing RPG level system plugins, such as data corruption (Desync), balancing difficulty, and intensive maintenance complexity.**

**이 플러그인은 데이터 오류(Desync), 밸런싱 난이도, 유지보수 복잡성 등의 문제점을 안정성을 최우선으로 하여 근본적으로 해결하는 데 중점을 두었습니다.**

### 1.1. Data Integrity Assurance: Solving Data Inconsistency Issues

### 1.1. 데이터 무결성 보장: 데이터 불일치 (`Desync`) 문제 해결

* Utilizing a centralized cache system, all player data is consistently stored in the memory cache, and robust loading/saving features minimize data loss during unexpected server shutdowns.

* 중앙 집중식 캐시 시스템을 이용해 모든 플레이어 데이터는 메모리 캐시에 일관성 있게 보관되며, 강력한 불러오기 및 저장 기능으로 데이터 유실을 최소화합니다.
...
* 전체 데이터베이스 연결 및 저장 로직은 **비동기적**으로 처리되도록 설계되어, I/O 부하가 높을 때도 서버의 메인 스레드를 막지 않아 서버 지연(Lag)을 방지합니다.
...
**DRPG_levelSystem은 모듈식 데이터 기반 아키텍처를 기반으로 하여, 높은 유연성과 확장 용이성을 갖도록 설계되었습니다.**
...
* 스킬 경험치 트리거와 성장 로직은 `plugins/DRPG/levelSystem/skills/`에 위치한 JSON 파일 내에서 완전히 정의됩니다.
...
* 인게임 특정 이벤트에서 얻는 경험치가 정확하게 계산되고, 레벨업이 정확하게 처리되며, 도달한 레벨마다 미리 정의된 보상이 즉시 지급되도록 보장합니다.
...
* 레벨 업 시 지급되는 보상은 구조화된 JSON 배열(`rewards/*.json`)을 이용해 레벨별로 분리되어 관리됩니다.
...
* `nbt` 필드를 활용하여 보상으로 정의된 아이템에 커스텀 데이터, 이름, 설명(Lore), 강력한 속성을 추가할 수 있는 확장성을 확보하였습니다.
...
* PlaceholderAPI와의 완벽한 연동을 지원하여, 레벨, 경험치 및 기타 동적 데이터를 스코어보드, 채팅, 다양한 UI 요소에 표시할 수 있습니다.
...
* 주요 권한 플러그인(예: LuckPerms)과 연동되어 스킬 사용 및 명령어 실행에 대한 세밀한 권한 제어를 제공합니다.
...
* 플러그인은 `config.json` 및 모든 스킬/보상 정의 파일의 구문 및 구조를 사전에 검사하며, 단순한 오류에 대해서는 **자동 수정**을 시도하고 치명적인 오류 발생 시 그 원인을 상세하게 보고합니다.
...
* `config.json`의 `Debug` 설정을 활성화하면 파일 로드, 파싱, 등록, 그리고 구체적인 오류 원인에 대한 상세 로그가 출력되어 문제 해결 시간을 획기적으로 단축시킵니다.
...
* 예상치 못한 치명적인 예외가 발생했을 때 서버 다운을 방지하고 관리자에게만 경고와 보고서를 제공하는 **안전 모드** 기능을 포함하고 있습니다.
