    ...
    "BlockPlace": [ // 플레이어가 블록을 설치했을 때 발생하는 이벤트
      {
        ...
        "EXP": {
          "min": int, // 최소 지급 EXP
          "max": int  // 최대 지급 EXP (min과 같으면 고정값)
        },

        "PlayerGetExp": int,
        // 플레이어에게 직접 지급되는 EXP 값
        // (스킬 EXP와 별도로 처리 가능)
        ...
        "AdvancedSettings": {
          ...
          // 블록 데이터(BlockData)까지 정확히 일치해야 EXP 지급
          // (방향, 상태 등)
          ...
          "Requirements": {
            ...
            "SurfaceOnly": boolean,
            // true일 경우 지표면(Y 기준 노출 블록)에서만 허용
{
  "Events": {
    "BlockPlace": [ // 플레이어가 블록을 설치했을 때 발생하는 이벤트
      {
